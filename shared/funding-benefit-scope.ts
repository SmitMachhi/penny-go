...
const REJECTED_LOAN_CONTEXT_PATTERN =
	/\b(?:excluded by scope|outside scope|ruled out|skip|you said no loans|which you said no to|you said no to|you don['\u2019]t want|not what you asked for|not eligible|not a fit|doesn['\u2019]t fit|does not fit|what doesn['\u2019]t fit|what does not fit|weak fit|primarily a loan|loan[- ]first|is (?:primarily )?a loan(?: program)?|not the default(?: form)?)\b/i;
const INSTRUMENT_CATALOG_LOAN_CONTEXT_PATTERN =
	/\brepayable contributions?\s*\([^)]*\bloan/i;
...
function actionableLoanLikeMatch(text: string): RegExpExecArray | null {
	...
	if (INSTRUMENT_CATALOG_LOAN_CONTEXT_PATTERN.test(normalized)) {
		return null;
	}
